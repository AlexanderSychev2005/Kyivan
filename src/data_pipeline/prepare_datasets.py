...
def parse_year(year_str):
    if not year_str or str(year_str).lower() == "unknown": return None
...
def get_macro_dialect(dataset_name, dialect_str, file_source=''):
    ...
    if dataset_name in ['pushkin_texts', 'torot']:
        ...
        is_oes_base = 'древнерусск' in dialect or 'старорусск' in dialect or 'московск' in dialect or 'русском языке' in dialect or 'приказн' in dialect
        is_cs_base = 'церковнославянск' in dialect or 'старославянск' in dialect or 'среднеболгарск' in dialect or 'южнославянск' in dialect
        
        # Explicit starts
        if dialect.startswith('церковнославянский') or dialect.startswith('старославянский') or 'на старославянском' in dialect or 'на церковнославянском' in dialect:
            return 'CS'
            
        if 'влияни' in dialect or 'элемент' in dialect or 'смешени' in dialect or 'традици' in dialect:
            if is_oes_base:
                return 'OES'
        ...
        with open(out_path, 'w', encoding='utf-8') as f_out:
            for doc in data:
                ...
                new_doc = {
                    ...
                    "date_target": target,
                    "date_number": raw_year if interval and interval[0] == interval[1] else None,
