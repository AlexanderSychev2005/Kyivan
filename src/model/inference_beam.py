"""
Kyivan Beam-Search Restorer (research/eval only -- not used by app.py).
...
KyivanRestorer (inference.py) rather than modifying it.

inference.py's own greedy decode loop was later extracted into
`KyivanRestorer.iterative_decode(tokens)` and beam_width>1 was dropped from
production serving (app.py always uses the greedy path -- nothing there
calls this module). This file still earns its keep for evaluate_cer.py's
--beam_width>1 mode: comparing greedy vs. real beam search on CER is a
useful ablation for the paper, matching how Aeneas itself only uses beam
search at eval time, never during training. If that comparison stops being
wanted, this file (and the beam_width path in evaluate_cer.py) can go too
-- greedy alone is covered by KyivanRestorer.iterative_decode without it.

Input syntax (see KyivanRestorer.tokenize): bare `?` for one missing
character, `#` for a lacuna of unknown length -- same convention as
inference.py, no brackets, no `[SOS]`.

Unlike KyivanRestorer.iterative_decode's greedy "fill the single most
confident mask, one at a time" loop, this explores many candidate
restorations in parallel: at each iteration every hypothesis in the beam
branches over EVERY still-open `[-]`/`[#]` position at once (all candidate
characters for `[-]`, both expand/stop for `[#]`), all resulting candidates
are scored by length-normalized log-probability, and only the top
`beam_width` survive to the next iteration. Returns the top-N full
candidate restorations, ranked, rather than a single greedy answer.
    ...
    def restore_text_beam(
        ...
        Args:
            text: The corrupted input string, using bare `?` for one missing
                ...
                "а се покл#е" or "т?а"). Ignored if `tokens` is given.
            tokens: Pre-tokenized input ids, bypassing tokenize() (and
                therefore normalize_historical_text) entirely. Needed by
                callers that already have exact token ids to preserve --
                e.g. evaluate_cer.py builds a query around ground-truth
                context on both sides of one lacuna; round-tripping that
                context through decode() -> text -> tokenize() isn't safe,
                since normalize_historical_text isn't guaranteed idempotent
                on already-normalized text (it can still match patterns
                like _HYPHEN_GAP_RE/_ELLIPSIS_RE in genuine historical text
                and alter it on a second pass). Exactly one of text/tokens
                must be given.
